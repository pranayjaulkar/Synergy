  ...
  return (
    <div
      className={`relative w-full flex items-center justify-between ${
        plain ? "" : " border-b border-b-zinc-100 dark:border-b-zinc-700 shadow"
      }`}
    >
      <div className="w-full flex items-center justify-between px-4 md:px-8 py-3 m-0 sticky top-0 min-h-16 h-16">
        <Link to="/" className="flex items-center space-x-4">
          ...
          <h1 className="text-lg font-medium dark:text-white">Synergy</h1>
        </Link>
        ...
        {!plain && (
          <>
            <div className="px-2 h-full w-fit lg:min-w-lg flex items-center border border-zinc-300 dark:border-zinc-700 rounded-full overflow-hidden">
              <Search className="hover-darken dark:text-white cursor-pointer p-1" size={32} />
              <input
                type="text"
                className="w-full h-full flex items-center outline-none px-3 dark:text-white dark:border-zinc-700 py-3 justify-center rounded-full"
              />
...
function Profile() {
  ...
  const hanldeClickOutside = (event: MouseEvent) => {
    if (userPopupRef.current && (event.target as HTMLDivElement)?.id !== "user-profile-icon")
      setIsUserPopupOpen(userPopupRef.current.contains(event.target as HTMLDivElement));
  };
